// KenyaHOA Pro - Development server with seeded data and Hono integration
...
let honoApp;
try {
  const honoModule = await import('./dist/_worker.js');
  honoApp = honoModule.default;
} catch (error) {
  console.warn('Could not load Hono app, using static files only:', error.message);
}
...
const server = http.createServer(async (req, res) => {
  ...
  // Handle root route with Hono app if available
  if (req.url === '/' && honoApp) {
    try {
      ...
      console.warn('Error using Hono app for root route:', error.message);
  ...
  if (url === '/api/health') {
    return {
      ...
      data: {
        ...
        hono_integration: honoApp ? 'enabled' : 'disabled'
      }
    };
  }
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 KenyaHOA Pro hybrid development server running on http://localhost:${PORT}`);
  console.log(`📊 Health check: http://localhost:${PORT}/api/health`);
  console.log(`🏠 Dashboard: http://localhost:${PORT}/dashboard`);
  console.log(`👥 Demo accounts: http://localhost:${PORT}/api/auth/demo-accounts`);
  console.log(`🔧 Hono integration: ${honoApp ? 'enabled' : 'disabled (fallback to static)'}`);
  console.log('');
